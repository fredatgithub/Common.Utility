...
namespace Common.Utility
{
  ...
  public static class DirFile
  {
    ...
    /// 范例："Log*.xml"表示搜索所有以Log开头的Xml文件。</param> 
    /// <param name="isSearchChild">是否搜索子目录</param>
    public static bool Contains(string directoryPath, string searchPattern, bool searchChild = false)
    {
      try
      {
        //获取指定的文件列表
    ...
    public static void CreateFile(string dir, string pagestr)
    {
      ...
      StreamWriter sw = new StreamWriter(System.Web.HttpContext.Current.Request.PhysicalApplicationPath + "\\" + dir, false, Encoding.GetEncoding("GB2312"));
      sw.Write(pagestr);
      sw.Close();
    }
    #endregion
    ...
    public static void Move(string sourceFilePath, string descDirectoryPath)
    {
      ...
      if (DoesDirectoryExist(descDirectoryPath))
      {
        //如果目标中存在同名文件,则删除
        if (DoesFileExist(descDirectoryPath + "\\" + sourceFileName))
        {
